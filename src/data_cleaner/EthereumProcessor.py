...
class EthereumProcessor:
    ...
    def combine_dataframes(self) -> pd.DataFrame:
        ...
        """
        combined_price = pd.concat([self.price_dataframe1, self.price_dataframe2], ignore_index=True) # Concatenate the price dataframes
        combined_price = combined_price.groupby('timestamp').mean() # Get the mean price across the price dataframes (grouped by timestamp)

        combined_dataframe = pd.merge(combined_price, self.news_dataframe, on='timestamp', how='inner') # Combine the news and price dataframes
