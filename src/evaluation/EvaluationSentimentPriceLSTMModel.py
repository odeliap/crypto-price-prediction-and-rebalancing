...
def split_sequences(
    ...
    for i in range(len(input_sequences)):
        ...
        output_end_index = end_index + n_steps_out - 1
        # Check if we are beyond the dataset
        if output_end_index > len(input_sequences): break
...
def main(coin: str, filepath: str, n_steps_in: int, n_steps_out: int) -> None:
    ...
    lstm = EvaluationSentimentPriceLSTM(n_steps_out,
                input_size,
                hidden_size,
                num_layers)
    ...
    plt.title(f'{coin.capitalize()} Static-Sentiment Time-Series Prediction')
    plt.legend()
    plt.savefig(f"outputs/graphs/SentimentPriceLSTMModel_static_sentiment_whole_plot_{coin}.png", dpi=300)
    plt.show()
    ...
    plt.xlabel("Time[Days]")
    plt.ylabel("Price (USD)")
    plt.title(f'{coin.capitalize()} Static-Sentiment LSTM Predictions vs Actual Data')
    plt.legend()
    plt.savefig(f"outputs/graphs/SentimentPriceLSTMModel_static_sentiment_small_plot_{coin}.png", dpi=300)
    plt.show()
    ...
    plt.xlabel("Time[Days]")
    plt.ylabel("Price (USD)")
    plt.title(f"{coin.capitalize()} Static-Sentiment One-Shot Multi-Step Prediction ({n_steps_out} Days)")
    plt.legend()
